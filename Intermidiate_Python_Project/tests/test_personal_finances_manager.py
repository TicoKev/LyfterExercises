...
from project_logic import filter_transaction, calculate_account_balance, date_handler
...
def make_transactions(title="T", category="C", amount=10, quantity=1, date="01/01/2025", tx_type="Expense", color="#000000"):
    return SimpleNamespace(
        ...
        transaction_date=datetime.strptime(date, "%d/%m/%Y").date(),  # 👈 ahora es date
...
def test_filter_transaction_with_valid_dates_returns_list():
    ...
    start = datetime.strptime("01/01/2025", "%d/%m/%Y").date()
    end = datetime.strptime("31/12/2025", "%d/%m/%Y").date()
...
def test_filter_transaction_no_data_raises_valueerror():
    ...
    start = datetime.strptime("01/01/2026", "%d/%m/%Y").date()
    end = datetime.strptime("31/12/2026", "%d/%m/%Y").date()
...
def test_format_date_valid_and_invalid():
    ...
    assert isinstance(parsed, date)  # ahora correcto
    invalid = "31/02/2025"
...
def test_calculate_account_balance_numeric_and_non_numeric():
    ...
    with pytest.raises(ValueError):
        calculate_account_balance.calculate_account_balance("not-a-number", None)
