...
def create_student(students_list):
  ...
  for new_student in range(students_quantity):
    name = input(f"Enter full name for student #{new_student + 1}: ").strip()
    is_valid_name(name)
    section = input(f"Enter {name}`s class: ").strip()
    is_valid_section(section)
    
    duplicate_student(students_list, name, section)
...
def print_students_info(students_list):
  for student in students_list:
      print(f"Name: {student.name}, Section: {student.section}, Grades: {student.grades}")


def grades_average(students_list):
    try:
...
def get_top_3_students(students_list):
  ...
  for student in students_list:
    try:
      ...
      print(f"The key {e} is missing in student: {student}")
    except TypeError:
